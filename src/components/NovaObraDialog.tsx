...
import { criarObra, uploadArquivoBIM } from "@/services/api";
...
const NovaObraDialog = ({ open, onOpenChange, onObraCreated }: NovaObraDialogProps) => {
  ...
  const handleSubmit = async () => {
    ...
    const resultadoObra = await criarObra(dadosObra);
    ...
    // Upload do arquivo BIM
    const obraId = resultadoObra.data.id_obra;
    const resultadoUpload = await uploadArquivoBIM(obraId, selectedFile);

    if (resultadoUpload.error) {
      toast.error("Obra criada, mas erro ao enviar arquivo", {
        description: resultadoUpload.error,
      });
    } else {
